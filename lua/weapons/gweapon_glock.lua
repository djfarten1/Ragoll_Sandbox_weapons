...
SWEP.WeaponModel = "models/weapons/w_pist_glock18.mdl"
...
hook.Add("KeyPress", "CustomKeyPress", function(ply, key)
	if key == IN_USE and ply:KeyDown(IN_ATTACK2) and ply:GetActiveWeapon():GetClass() == "gweapon_pistol" then
		local steamID = ply:SteamID()
        if attachment_state[steamID] == nil or not attachment_state[steamID] then
            attachment_state[steamID] = true
        else
            attachment_state[steamID] = false
        end
	end
end)
--AutoHook Table
...
table.Merge(SWEP,{
	...
	CreateWeaponEnt = function(self)
		...
		if not attachment_state[ply:SteamID()] then 
		wep:SetModel(self.WeaponModel)
		else
		wep:SetModel("models/weapons/w_pist_glock18.mdl")
		end 
		wep:Spawn()
		wep:Activate()
	...
	ShootEffects = function(self)
		...
		if not attachment_state[ply:SteamID()] then
		self.WeaponEnt:EmitSound("weapons/glock/glock18-1.wav",75,100,1,CHAN_WEAPON)
		else
		self.WeaponEnt:EmitSound("weapons/glock/glock18-1.wav",75,100,1,CHAN_WEAPON)
		end
	end,
	...
	Reload = function(self)
		if(SERVER)then
			...
			if(self:Clip1()<self:GetMaxClip1() and ply:GetAmmoCount( self:GetPrimaryAmmoType() )>0 and !self.Reloading)then
				...
				self.WeaponEnt:EmitSound("weapons/glock/glock_clipin.wav",55,100,1,CHAN_AUTO)
			end
		end
	end,
	...
	Shoot = function(self)
		...
		local spread = 0.01 * self:GetRecoilMul()
